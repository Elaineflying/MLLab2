...
train_mse <- mean((train_data$Fat - train_pred)^2)

test_pred <- predict(lm_model, newdata = test_data)
test_mse <- mean((test_data$Fat - test_pred)^2)
...
plot(test_data$Fat, test_predict, xlab = "Original test values", ylab = "Predicted test values")
